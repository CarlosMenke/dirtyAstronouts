...
def find_dirtiest_datacenter(datacenters: list):
    ...
    logger.debug(dirty_datacenter)
    exists_clean_datacenter = False
    for datacenter in datacenters:
        dirty_power = datacenter.dirty_power()
        if dirty_power < 0:
    ...
    if dirty_datacenter.dirty_power() < 0:
        logger.debug(f"No dirty datacenter found .. {dirty_datacenter.dirty_power()}")
        return False
...
def move_vm(datacenters: list, dirty_datacenter: Datacenter, vm: VM):
    ...
    dirty_datacenter.vms.remove(vm)
